    ...
    try {
        ...
        if ($rowCount > 0) {
            ...
            try {
                ...
                header('Location: ../../administrator/alumni-declined.php?type=success&message=Emails have been sent successfully.');
            } catch (Exception $e) {
                echo "Email could not be sent. Mailer Error: {$mail->ErrorInfo}";
                header('Location: ../../administrator/alumni-declined.php?type=error&message=Email could not be sent. Mailer Error:');
            }
        } else {
            echo 'User status update failed.';
            header('Location: ../../administrator/alumni-declined.php?type=error&message=Couldnt find your email.');
        }
    } catch (PDOException $e) {
        // Handle any database-related errors
        echo 'Database error: ' . $e->getMessage();
        header('Location: ../../administrator/alumni-declined.php?type=error&message=Database error.');
    }
} else {
    echo 'Invalid request. Missing user ID or email.';
    header('Location: ../../administrator/alumni-declined.php?type=error&message=Invalid request. Missing user ID or email.');
}
